...
top_connections: list[tuple[int, int]] = list(map(lambda d: d[1], sorted(distances, key=lambda d: d[0])))[:connections]

for a, b in top_connections:
...
result = 1
for l in sorted(list(map(lambda net: len(net), nets)), reverse=True)[:3]:
    result*=l
print(result)
